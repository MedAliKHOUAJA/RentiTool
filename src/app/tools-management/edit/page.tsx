...
const EditToolPageContent = () => {
  ...
  const [preStatusEdit, setPreStatusEdit] = useState<
    "idle" | "checking" | "ok" | "fraud" | "error"
  >("idle");
  const [preMessageEdit, setPreMessageEdit] = useState<string | null>(null);
  const [preSimilarityEdit, setPreSimilarityEdit] = useState<number | null>(
    null
  );
    ...
    const run = async () => {
      try {
        ...
        setRentalPricePerDay(
          tool.rentalPricePerDay ? String(tool.rentalPricePerDay).replace(/[^0-9.]/g, "") : ""
        );
        setCategoryId(
          tool.listingCategory?.id ? String(tool.listingCategory.id) : ""
        );
        setSubCategoryId("");
      } catch (e: any) {
  ...
  // Debounced live fraud precheck for Edit form
  useEffect(() => {
    const hasText = title.trim().length + description.trim().length > 0;
    const hasImage = images.length > 0;
    if (!hasText || !hasImage) {
      setPreStatusEdit("idle");
      setPreMessageEdit(null);
      setPreSimilarityEdit(null);
      return;
    }
    setPreStatusEdit("checking");
    setPreMessageEdit(null);
    setPreSimilarityEdit(null);
      ...
      try {
        ...
        if (!pre.ok) {
          ...
          if (
            ...
            setPreSimilarityEdit(!Number.isNaN(sim) ? sim : null);
            setPreStatusEdit("fraud");
            setPreMessageEdit(
              preJson?.message || "Image does not match title/description."
            );
            return;
          }
          setPreStatusEdit("error");
          setPreMessageEdit(preJson?.message || "Fraud check failed");
          return;
        }
        setPreStatusEdit("ok");
        setPreMessageEdit(null);
        setPreSimilarityEdit(preJson?.similarity ?? null);
      } catch (e: any) {
        setPreStatusEdit("error");
        setPreMessageEdit(e?.message || "Fraud check failed");
      }
    }, 700);
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      // Fraud-check before updating, using first image URL if available
      try {
        const fd = new FormData();
        fd.append("title", title.trim() || "");
        fd.append("description", description.trim() || "");
        ...
        if (!pre.ok) {
          ...
          if (
            ...
            setError(
              "The current primary image doesn’t appear to match the title/description. Try another image or adjust the text."
            );
            setSaving(false);
            return;
          }
        ...
        <div className="lg:flex lg:space-x-10 mt-8">
          <form onSubmit={onSubmit} className="w-full lg:w-2/3 space-y-8">
            ...
            <div className="listingSection__wrap rounded-3xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 p-6 lg:p-8">
              ...
              {preStatusEdit === "checking" && (
                <Alert variant="info" title="Checking image/text match…" />
              )}
              {preStatusEdit === "fraud" && (
                <Alert variant="error" title="Potential mismatch detected">
                  <div className="whitespace-pre-line">{preMessageEdit}</div>
                  {preSimilarityEdit !== null && (
                    <div className="mt-3 flex items-center justify-center">
                      <SimilarityCircle
                        similarity={preSimilarityEdit}
                        size={88}
                      />
                    </div>
                  )}
              ...
              {preStatusEdit === "ok" && preSimilarityEdit !== null && (
          ...
          <div className="w-full lg:w-1/3 mt-8 lg:mt-0">
            <div className="listingSectionSidebar__wrap rounded-3xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 p-6 lg:p-8">
              ...
              <button
                disabled={saving || preStatusEdit === "fraud"}
