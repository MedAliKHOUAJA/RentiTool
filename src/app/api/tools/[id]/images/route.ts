...
const resolveTable = async (candidates: string[]) => {
  ...
  for (const cand of candidates) {
    ...
    const colsRes = await query(`SELECT column_name FROM information_schema.columns WHERE table_schema=$1 AND table_name=$2`, [schema, table]);
    if (colsRes.rows.length) return { schema, table, columns: colsRes.rows.map((r: any) => r.column_name as string) };
  }
  return null;
};
...
export async function GET(
  ...
  try {
    ...
    if (!imgResolved) return new NextResponse('Images table not found', { status: 500 });
    const { schema: ischema, table: itable, columns: icols } = imgResolved;
    const lowerMap: Record<string,string> = Object.fromEntries(icols.map(c => [c.toLowerCase(), c]));
    const has = (...names: string[]) => names.map(n => lowerMap[n.toLowerCase()]).find(Boolean);
    const pk = has('imageid','id','image_id');
    const toolCol = has('toolid','tool_id');
    const isPrimary = has('isprimarytoolimage','is_primary','isprimary');
    if (!pk || !toolCol) return new NextResponse('Images schema invalid', { status: 500 });
    const q = (s: string) => `"${s}"`;
    const from = `${q(ischema)}.${q(itable)}`;
    const sql = `SELECT ${q(pk)} as id, ${q(isPrimary || pk)} as primary FROM ${from} WHERE ${q(toolCol)} = $1 ORDER BY ${q(pk)} DESC LIMIT 50`;
    const res = await query(sql, [toolId]);
    const images = res.rows.map((r: any) => ({ id: r.id, isPrimary: Boolean(r.primary), url: `/api/images/${r.id}` }));
...
export async function POST(
  ...
  try {
    ...
    const toolCol = has('toolid','tool_id');
    const isPrimary = has('isprimarytoolimage','is_primary','isprimary');
    if (!pk || !bin || !toolCol) return new NextResponse('Images schema invalid', { status: 500 });
    const q = (s: string) => `"${s}"`;
    const from = `${q(ischema)}.${q(itable)}`;
    ...
    if (isPrimary) {
      const check = await query(`SELECT 1 FROM ${from} WHERE ${q(toolCol)}=$1 LIMIT 1`, [toolId]);
    ...
    const sql = `INSERT INTO ${from} (${cols.join(',')}) VALUES (${placeholders.join(',')}) RETURNING ${q(pk)} as id`;
    const ins = await query(sql, values);
    const idNew = ins.rows[0]?.id;
    return NextResponse.json({ id: idNew, url: `/api/images/${idNew}`, isPrimary: makePrimary }, { status: 201 });
  } catch (err: any) {
