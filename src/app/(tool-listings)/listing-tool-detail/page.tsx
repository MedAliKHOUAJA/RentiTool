...
const ToolDetailPageContent = () => {
  ...
  const { title, address, reviewStart, reviewCount, price, saleOff, desc, author, galleryImgs } = tool;
  ...
  const priceNumber = Number(String(price).replace(/[^0-9.,]/g, '').replace(',', '.')) || 0;
  const priceFormatted = new Intl.NumberFormat('fr-FR', { maximumFractionDigits: 0 }).format(priceNumber);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10">
          ...
          <div className="lg:col-span-5">
              ...
              <div>
                <div className="flex flex-wrap items-center gap-3 mb-3">
                  {tool.listingCategory?.name && (
                    <span className="inline-flex items-center px-3 py-1 rounded-full text-xs bg-neutral-100 dark:bg-neutral-800 text-neutral-700 dark:text-neutral-300">{tool.listingCategory.name}</span>
                  )}
                </div>
                ...
                <div className="mt-3 flex flex-wrap items-center gap-4 text-sm text-neutral-500 dark:text-neutral-400">
                  <div className="flex items-center gap-2">
                    <StartRating reviewCount={reviewCount ?? 0} point={reviewStart ?? 0} />
                    <span>({reviewCount ?? 0} reviews)</span>
                  </div>
                  {address && <span className="truncate">{address}</span>}
                  {saleOff && <span className="text-red-500 font-medium">{saleOff}</span>}
                </div>
              </div>

              <div className="mt-6 flex items-start justify-between">
                <div>
                  <div className="text-red-600 dark:text-red-500 text-3xl md:text-4xl font-extrabold tracking-tight">{priceFormatted} DT</div>
                  <div className="mt-2 text-sm">Availability: <span className={isAvailable ? 'text-green-600' : 'text-red-600'}>{isAvailable ? 'In stock' : 'Unavailable'}</span></div>
                </div>
              </div>
            ...
            transition duration-200 hover:-translate-y-0.5 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-neutral-300 dark:focus:ring-neutral-600 active:translate-y-0"
                  ...
                  <div className="mt-4 text-xs text-neutral-500 dark:text-neutral-400">Free cancellation within 24h • Support 7/7</div>
                </>
              )}
            </div>
          </div>
          ...
          <div className="lg:col-span-2">
            <div className="rounded-3xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 p-6 lg:p-8">
              ...
              <div className="mt-4 prose prose-neutral dark:prose-invert max-w-none">
                <p className="leading-relaxed">{desc || 'Aucune description fournie pour cet outil.'}</p>
              </div>
            </div>
          </div>
          <div>
            <div className="rounded-3xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 p-6 lg:p-8">
              <h2 className="text-xl font-semibold mb-4">À propos du propriétaire</h2>
